...
1. **Litany library** — a curated, alphabetically sorted collection of WH40k
   ...
   *Quotes* sections), filterable by category and full-text search.
2. **Custom text & document import** — add your own text layers, or import
   `.txt` / `.docx` files. Formatting is reduced to clean line breaks so the
   text drops straight into a layer (`.docx` is parsed with
...
4. **Background sizing & cropping** — load a background image and define the
   source crop rectangle that is scaled to *cover* the 1024² canvas. Add a
...
- **Remote images & CORS**: images added by URL are loaded with
  ...
  headers, the image either fails to load or *taints* the canvas, which makes
